...
// Type: OnyxServer.CLASSES.Player
...
namespace WARTLS.CLASSES
{
  public class Player
  {
    public RoomProperties RoomPlayer = new RoomProperties();
    public ClanProperties ClanPlayer = new ClanProperties();
    public Clan Clan = new Clan();
    public long UserID = -1;
    public string Nickname = "NoNameYet";
    public string Gender = "male";
    public long LastSeen = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
    public DateTime LastReceiveBonus = DateTimeOffset.FromUnixTimeSeconds(0).Date;
    public XmlDocument Settings = new XmlDocument();
    public XmlDocument Stats = new XmlDocument();
    public XmlDocument RandomBox = new XmlDocument();
    public XmlDocument Achievements = new XmlDocument();
    public XmlDocument notifications = new XmlDocument();
    public XmlDocument friends = new XmlDocument();
    public double Height = 1.0;
    public double Fatness = 1.0;
    public string Head = "default_head_04";
    public string BannerMark = "0";
    public string BannerBadge = "0";
    public string BannerStripe = "0";
    public int GameMoney = 150000;
    public int CryMoney = 12900;
    public int CrownMoney = 5400;
    public Random Random = new Random();
    public bool SoldierPassed = true;
    public bool EngineerPassed = true;
    public bool MedicPassed = true;
    public bool SoldierSuggest = true;
    public bool EngineerSuggest = true;
    public bool MedicSuggest = true;
    public string UnlockedMissions = "none,trainingmission,all,easy,normal,hard";
    public List<Item> Items = Core.GameResources.NewbieItems;
    public Stopwatch BoxWatcher = new Stopwatch();
    public BanType BanType;
    public long UnbanTime;
    public long UnmuteTime;
    public PrivilegieId Privilegie;
    public long TicketId;
    public byte OldRank;
    public byte CurrentClass;
    public int Experience;
    public int CurrentBonusPos;
    public byte CurrentBonusStreak;

        public int Observer;
    public StatsManager StatMgr;
    public int BoxCount;
    ...
    public byte Rank
    {
      get
      {
        foreach (XmlNode childNode in Core.GameResources.ExpCurve["exp_curve"].ChildNodes)
        {
          ...
          try
          {
            ...
            else
            {
              ...
              if (player.Load(false).Result)
              {
                ...
                xelement2.Add((object) new XAttribute((XName) "status", (object) 2));
    ...
    public XmlDocument Notifications
    {
      get
      {
    ...
    public XElement Avatar
    {
      get
      {
        ...
        xelement.Add((object) new XAttribute((XName) "clan_id", (object) this.Clan.ID));
        xelement.Add((object) new XAttribute((XName) "observer", (object) this.Observer));
        xelement.Add((object) new XAttribute((XName) "current_reward", (object) this.CurrentBonusPos));
        xelement.Add((object) new XAttribute((XName) "current_streak", (object) this.CurrentBonusStreak));
        return xelement;
      }
    }

    public void UpdateAchievement(XmlNode Chunk)
    {
    ...
    public void CheckAndFixItems()
    {
      ...
      if (all1.Count != 1 || all1[0].SecondsLeft < 0L)
      {
        ...
        Item obj1 = this.Items.Find((Predicate<Item>) (Attribute => Attribute.ItemType == ItemType.DEFAULT && Attribute.Name.StartsWith("ar") || Attribute.Name.StartsWith("mg")));
      ...
      if (all2.Count != 1 || all2[0].SecondsLeft < 0L)
      {
        ...
        Item obj1 = this.Items.Find((Predicate<Item>) (Attribute => Attribute.ItemType == ItemType.DEFAULT && Attribute.Name.StartsWith("shg")));
      ...
      if (all3.Count != 1 || all3[0].SecondsLeft < 0L)
      {
        ...
        Item obj1 = this.Items.Find((Predicate<Item>) (Attribute => Attribute.ItemType == ItemType.DEFAULT && Attribute.Name.StartsWith("smg")));
      ...
      Item obj2 = this.Items.Find((Predicate<Item>) (Attribute => Attribute.ItemType == ItemType.DEFAULT && Attribute.Name.StartsWith("smg")));
      obj2.Equipped = (byte) 4;
      obj2.Slot = 1024;
    }

    public XElement CheckDailyBonus()
    {
      TimeSpan timeSpan = DateTime.UtcNow.Subtract(DateTimeOffset.FromUnixTimeSeconds(this.LastSeen).Date);
      if (timeSpan.TotalDays > 0.0)
      {
        if (timeSpan.TotalDays > 1.0)
          this.CurrentBonusPos = 0;
        XElement xelement = XElement.Load("Gamefiles/Configs.xml");
        if ((int) this.CurrentBonusStreak >= xelement.Element((XName) "consecutive_login_bonus").Elements().ToList<XElement>().Count)
          this.CurrentBonusStreak = (byte) 0;
        List<XElement> list = xelement.Element((XName) "consecutive_login_bonus").Elements().ToList<XElement>()[(int) this.CurrentBonusStreak].Elements().ToList<XElement>();
        if (this.CurrentBonusPos >= list.Count)
        {
          this.CurrentBonusPos = 0;
          ++this.CurrentBonusStreak;
          if ((int) this.CurrentBonusStreak >= xelement.Element((XName) "consecutive_login_bonus").Elements().ToList<XElement>().Count)
            this.CurrentBonusStreak = (byte) 0;
          list = xelement.Element((XName) "consecutive_login_bonus").Elements().ToList<XElement>()[(int) this.CurrentBonusStreak].Elements().ToList<XElement>();
        }
        string EventName = list[this.CurrentBonusPos].Attribute((XName) "name").Value;
        XElement firstNode = (XElement) xelement.Element((XName) "special_reward_configuration").Elements().ToList<XElement>().Find((Predicate<XElement>) (a => a.Attribute((XName) "name").Value == EventName)).FirstNode;
        switch (firstNode.Name.LocalName)
        {
          case "money":
            int Amount = int.Parse(firstNode.Attribute((XName) "amount").Value);
            string Currency = firstNode.Attribute((XName) "currency").Value;
            if (Currency != null)
            {
              if (!(Currency == "cry_money"))
              {
                if (!(Currency == "crown_money"))
                {
                  if (Currency == "game_money")
                    this.CryMoney += Amount;
                }
                else
                  this.CrownMoney += Amount;
              }
              else
                this.GameMoney += Amount;
            }
            return this.AddMoneyNotification(Currency, Amount, "", true);
          case "item":
            string Name = firstNode.Attribute((XName) "name").Value;
            if (!Name.Contains("box") && !Name.Contains("coin"))
            {
              XAttribute xattribute = firstNode.Attribute((XName) "expiration");
              long num;
              ...
              else
              {
                ...
                num = utcNow.ToUnixTimeSeconds() + Tools.GetTotalSeconds(xattribute.Value);
              }
              Item obj1 = new Item();
              obj1.Name = Name;
              obj1.ExpirationTime = num;
              utcNow = DateTimeOffset.UtcNow;
              obj1.BuyTime = utcNow.ToUnixTimeSeconds();
              obj1.Quantity = (int) Tools.GetTotalSeconds(firstNode.Attribute((XName) "amount").Value);
              obj1.DurabilityPoints = firstNode.Attribute((XName) "amount") != null || firstNode.Attribute((XName) "expiration") != null ? -1L : 36000L;
              obj1.TotalDurabilityPoints = firstNode.Attribute((XName) "amount") != null || firstNode.Attribute((XName) "expiration") != null ? -1L : 36000L;
              Item obj2 = obj1;
              this.AddItem(obj2);
              return this.AddItemNotification(obj2.ItemType == ItemType.TIME ? "Expiration" : (obj2.ItemType == ItemType.DEFAULT ? "Consumable" : (obj2.ItemType == ItemType.CONSUMABLE ? "Permanent" : "Durability")), Name, obj2.Quantity, "", true);
            }
            break;
        }
      }
    ...
    {
      this.Items.Remove(this.Items.Find((Predicate<Item>) (Attribute => Attribute.ID == ItemID)));
    }

    public Player()
    {
      this.Items = new List<Item>((IEnumerable<Item>) Core.GameResources.NewbieItems);
    ...
    public XElement AddMoneyNotification(
      string Currency,
      int Amount = 0,
      string Message = "",
      bool isBonus = false)
    {
      ...
      xelement1.Add((object) new XAttribute((XName) "from_jid", (object) "onyx@server"));
      ...
      xelement2.Add((object) new XAttribute((XName) "type", (object) (isBonus ? 1 : 0)));
      xelement2.Add((object) new XAttribute((XName) "amount", (object) Amount));
      xelement2.Add((object) new XAttribute((XName) "notify", (object) "1"));
      if (isBonus)
      {
        XElement xelement3 = new XElement((XName) "consecutive_login_bonus");
        xelement3.Add((object) new XAttribute((XName) "previous_streak", (object) this.CurrentBonusStreak));
        xelement3.Add((object) new XAttribute((XName) "previous_reward", (object) (this.CurrentBonusPos - 1)));
        xelement3.Add((object) new XAttribute((XName) "current_streak", (object) this.CurrentBonusStreak));
        xelement3.Add((object) new XAttribute((XName) "current_reward", (object) this.CurrentBonusPos));
      ...
      string ClanName = "")
    {
      XElement xelement1 = new XElement((XName) "notif");
      xelement1.Add((object) new XAttribute((XName) "id", (object) this.Random.Next(999999, int.MaxValue)));
      xelement1.Add((object) new XAttribute((XName) "type", (object) 64));
      xelement1.Add((object) new XAttribute((XName) "confirmation", (object) 1));
      xelement1.Add((object) new XAttribute((XName) "from_jid", (object) "onyx@server"));
      xelement1.Add((object) new XAttribute((XName) "message", (object) ""));
      XElement xelement2 = new XElement((XName) "invitation");
      xelement2.Add((object) new XAttribute((XName) nameof (target), (object) target));
      ...
      XElement xelement3 = new XElement((XName) "initiator_info");
      xelement3.Add((object) new XAttribute((XName) "online_id", (object) onlineid));
      xelement3.Add((object) new XAttribute((XName) "profile_id", (object) profileid));
      xelement3.Add((object) new XAttribute((XName) "is_online", (object) "1"));
      xelement3.Add((object) new XAttribute((XName) "name", (object) Initiator));
      xelement3.Add((object) new XAttribute((XName) "clan_name", (object) ClanName));
      xelement3.Add((object) new XAttribute((XName) "experience", (object) Exp));
      xelement3.Add((object) new XAttribute((XName) nameof (badge), (object) badge));
      xelement3.Add((object) new XAttribute((XName) nameof (mark), (object) mark));
      xelement3.Add((object) new XAttribute((XName) nameof (stripe), (object) stripe));
      xelement2.Add((object) xelement3);
      xelement1.Add((object) xelement2);
      XmlReader reader = xelement1.CreateReader();
      reader.Read();
    ...
    public void AddClanNotification(
      string Initiator,
      string target,
      long profileid,
      string onlineid,
    ...
    public void AddCustomMessage(string Message)
    {
      ...
      xelement.Add((object) new XAttribute((XName) "from_jid", (object) "onyx@server"));
    ...
    public void AddResultNotification(
      long ProfileId,
      string Jid,
      string Nickname,
      int Status,
    ...
    public XElement AddItemNotification(
      string OfferType,
      string Name,
      int Amount = 0,
      string Message = "",
      bool isBonus = false)
    {
      ...
      if (isBonus)
      {
        XElement xelement3 = new XElement((XName) "consecutive_login_bonus");
        xelement3.Add((object) new XAttribute((XName) "previous_streak", (object) this.CurrentBonusStreak));
        xelement3.Add((object) new XAttribute((XName) "previous_reward", (object) this.CurrentBonusPos));
        xelement3.Add((object) new XAttribute((XName) "current_streak", (object) this.CurrentBonusStreak));
        ++this.CurrentBonusPos;
        xelement3.Add((object) new XAttribute((XName) "current_reward", (object) this.CurrentBonusPos));
        xelement2.Add((object) xelement3);
        xelement1.Add((object) xelement2);
        return xelement1;
    ...
    public void AddRandomBoxNotification(string Box, string Message = "", bool isBonus = false)
    {
      XElement xelement1 = new XElement((XName) "notif");
      xelement1.Add((object) new XAttribute((XName) "id", (object) this.Random.Next(999999, int.MaxValue)));
      ...
      {
        XElement xelement3 = new XElement((XName) "consecutive_login_bonus");
        xelement3.Add((object) new XAttribute((XName) "previous_streak", (object) ((int) this.CurrentBonusStreak - 1)));
        xelement3.Add((object) new XAttribute((XName) "previous_reward", (object) this.CurrentBonusPos));
        xelement3.Add((object) new XAttribute((XName) "current_streak", (object) this.CurrentBonusStreak));
        xelement3.Add((object) new XAttribute((XName) "current_reward", (object) this.CurrentBonusPos++));
        xelement2.Add((object) xelement3);
      ...
      foreach (XElement priz in this.GeneratePrizes(Box, out int _, 0))
        xelement4.Add((object) priz);
    ...
    public void AddRankNotifierNotification(byte OldRank, byte NewRank, string Message = "")
    {
      XElement xelement1 = new XElement((XName) "notif");
      xelement1.Add((object) new XAttribute((XName) "id", (object) this.Random.Next(999999, int.MaxValue)));
      ...
      return Item;
        ...
        public void Save()
        {
            using (MySqlConnection result = SQL.GetConnection().GetAwaiter().GetResult())
            {
               ...
               this.LastSeen = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
                new MySqlCommand(string.Format("UPDATE players SET LastActivity='{0}',Friends='{1}',Notifications='{2}',Achievements='{3}',Stats='{4}',Settings='{5}',Experience='{6}', Avatar='{7}',Items='{8}',RandomBox='{9}',PrivilegieId='{10}',BanType='{11}',UnbanTime='{12}' WHERE  ID={13};", (object)this.LastSeen.ToString("yyyy-MM-ddTHH:mm:ss"), (object)this.friends.InnerXml, (object)this.Notifications.InnerXml, (object)this.Achievements.InnerXml, (object)this.Stats.InnerXml, (object)this.Settings.InnerXml, (object)this.Experience, (object)this.Avatar.ToString(SaveOptions.DisableFormatting), (object)xelement.ToString(SaveOptions.DisableFormatting), (object)this.RandomBox.InnerXml, (object)(byte)this.Privilegie, (object)(byte)this.BanType, (object)this.UnbanTime, (object)this.UserID), result).ExecuteNonQuery();
            }

        }

        public List<XElement> GeneratePrizes(string BoxName, out int ErrId, int OfferId = 0)
    {
      ErrId = 0;
      ...
      try
      {
        ...
        foreach (XmlNode childNode1 in xmlDocument["shop_item"]["random_box"].ChildNodes)
        {
          ...
          if (xmlNodeList2.Count > 0 && num1 % 1000 == 0 && num2 == (byte) 0)
          {
            ...
            xelement.Add((object) obj2.Serialize(false, 0));
          ...
          else if (xmlNode.Attributes["name"].InnerText == "game_money_item_01")
          {
            ...
            xelement.Add((object) new XAttribute((XName) "total", (object) this.GameMoney));
            xelement.Add((object) new XAttribute((XName) "offerId", (object) OfferId));
            xelementList.Add(xelement);
          }
          ...
          else
          {
            ...
            xelement.Add((object) new XAttribute((XName) "error_status", (object) "0"));
            xelement.Add((object) obj1.Serialize(false, 0));
            xelementList.Add(xelement);
          }
          ++num2;
        }
    ...
    public async Task<bool> Load(bool IncludeData = false)
    {
      using (MySqlConnection result = SQL.GetConnection().GetAwaiter().GetResult())
      {
        MySqlConnection.ClearAllPools();
        MySqlCommand mySqlCommand = new MySqlCommand();
        mySqlCommand.Connection = result;
        ...
        else
        {
          ...
          try
          {
            long num = long.Parse(new MySqlCommand("SELECT profileid FROM users WHERE nickname='" + this.Nickname + "';", result).ExecuteScalar().ToString());
        ...
        try
        {
          ...
          this.Nickname = new MySqlCommand(string.Format("SELECT nickname FROM users WHERE profileid='{0}';", (object) this.UserID), result).ExecuteScalarAsync().GetAwaiter().GetResult().ToString();
        }
        catch (Exception ex)
        {
          return false;
...
label_12:
        using (MySqlDataReader mySqlDataReader = mySqlCommand.ExecuteReader())
        {
          ...
          using (XmlReader xmlReader = XmlReader.Create((TextReader) new StringReader(mySqlDataReader.GetString(2))))
          {
            ...
            this.BannerBadge = xmlReader["banner_badge"].ToString();
            this.BannerMark = xmlReader["banner_mark"].ToString();
            this.BannerStripe = xmlReader["banner_stripe"].ToString();
            this.UnlockedMissions = xmlReader["unlocked_missions"].ToString();
            this.Clan.ID = Convert.ToInt64(xmlReader["clan_id"]);
            this.Observer = Convert.ToInt32(xmlReader["observer"]);
                    }
          if (IncludeData)
          {
            try
          ...
          this.LastSeen = mySqlDataReader.GetInt64(10);
