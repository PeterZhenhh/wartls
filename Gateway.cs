...
namespace WARTLS
{
	...
	internal class Gateway
	{
		...
		private void ReceiveAsync(object User1)
		{
			...
			{   if (client != null)
				{Console.WriteLine($"Игрок {client.Player.Nickname} отключился от сервера", ConsoleColor.Red);
				ArrayList.OnlineUsers.Remove(client);
				Console.Title = $"TLS (Online: {ArrayList.OnlineUsers.FindAll((Client Attribute) => !Attribute.Dedicated).Count})";
				}
